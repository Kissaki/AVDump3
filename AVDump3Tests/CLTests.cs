        ...
        foreach(var hashElem in testVectors.Elements("Hash"))
        {
            ...
            foreach(var testVectorElem in hashElem.Elements("TestVector"))
            {
                ...
                foreach(var hashAlg in hashAlgs)
                {
                    ...
                    Assert.IsFalse(b.SequenceEqual(bHash), "Source Data has been corrupted");
                    ...
                    Assert.IsTrue(hashStr.Equals(expectedHashStr), $"{hashName} - {hashAlg.GetType().Name} - {patternName} - {b.Length}: {hashStr} != {expectedHashStr}");
                    if(hashAlg.AdditionalHashes.IsEmpty)
                    {
                        Assert.IsNull(testVectorElem.Attribute("hash2"), message: $"{hashName} - {hashAlg.GetType().Name} - {patternName} - {b.Length}: Additional hashes expected");
                    }
                    else
                    {
